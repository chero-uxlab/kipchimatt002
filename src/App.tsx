  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col antialiased">
      ...
      {comparedProducts.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900/95 dark:bg-black/95 text-white p-4 rounded-2xl shadow-2xl flex items-center gap-6 z-40 border border-plum/40 max-w-[95vw] md:max-w-xl transition-all duration-300">
          <div className="flex-1">
            ...
            <span className="text-[10px] text-pink-200/90 font-bold">{comparedProducts.length} of 3 items selected</span>
            <div className="flex gap-2 mt-1.5">
              {comparedProducts.map(p => (
                <div key={p.id} className="relative group">
                  <img src={p.image} className="w-8 h-8 rounded-md object-cover border-2 border-plum" alt={p.name} />
                  <button 
                    onClick={() => handleToggleCompare(p)}
                    className="absolute -top-1 -right-1 bg-red text-white rounded-full w-4 h-4 text-[9px] flex items-center justify-center font-bold hover:bg-red-light cursor-pointer shadow-xs"
                  >
                    ×
                  </button>
                </div>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="bg-plum hover:bg-plum-dark text-white px-4 py-2 rounded-xl text-xs font-black tracking-wide uppercase shadow-lg shadow-plum/25 cursor-pointer transition-all active:scale-95 flex items-center gap-1.5"
            >
              <span>Compare</span>
            </button>
            <button 
